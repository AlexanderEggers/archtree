...
class BindableLinearLayout : LinearLayout {
    ...
    fun setAdapter(adapter: BinableLinearLayoutAdapter) {
        adapter.bindViewGroup(this)
    }

    private class ListChangedListener<T>
    internal constructor(private val adapter: BinableLinearLayoutAdapter) : ObservableList.OnListChangedCallback<ObservableList<T>>() {
    ...
    companion object {
        ...
        fun <T : BindableListItem, V : ViewModel> bindItemsSource(
            ...
            val baseListAdapter = object : BinableLinearLayoutAdapter() {

                override val itemCount: Int
                    get() = newItems?.size ?: 0

                override fun onCreateViewHolder(viewGroup: ViewGroup, type: Int): BinableLinearLayoutAdapter.ViewHolder {
                ...
                override fun onBindViewHolder(viewHolder: BinableLinearLayoutAdapter.ViewHolder, position: Int) {
            ...
            container.setAdapter(baseListAdapter)
        }

        private class DataContextAwareViewHolder<T : BindableListItem, V : ViewModel>
        internal constructor(private val binding: ViewDataBinding) : BinableLinearLayoutAdapter.ViewHolder(binding.root) {
